...
// AI Summary panel shows warning/watch names in lowercase, unlike the
// Issued Warnings & Watches panel which keeps Title Case from MetService.
function formatAlertNameLower(headline: string, isPlural: boolean = false) {
  return formatAlertName(headline, isPlural).toLowerCase();
}
    ...
    <div className="flex items-stretch gap-2">
      ...
      <span>
        {isMultipleAreas ? (
          ...
          <>
            A {formatAlertNameLower(issuedWarningOrAlert.headline, isMultipleAreas)}{' '}
            has been issued{' '}
            ...
            <div className="inline-flex justify-center items-baseline gap-1 relative top-0.5">
              ...
              <CopyIcon
                content={`A ${formatAlertNameLower(issuedWarningOrAlert.headline, isMultipleAreas)} has been issued ${issuedWarningOrAlert.areaDesc.length > 0 ? `for ${issuedWarningOrAlert.areaDesc}` : ''}${chance && upgradeTo ? `. There is a ${chance.toLowerCase()} confidence of upgrading to a ${upgradeTo}` : ''}.`}
              />
            </div>
          </>
        )}
...
function IssuedAlertsRemaining({
  ...
  const content = isMultipleAreas
    ? `The ${formatAlertNameLower(issuedWarningOrAlert.headline, isMultipleAreas)} remain in place for ${formatAreasList(allAreas)}.`
    : `The ${formatAlertNameLower(issuedWarningOrAlert.headline, isMultipleAreas)} remains in place for ${formatAreasList(allAreas)}.`;
  return (
    <div className="flex items-stretch gap-2">
...
function IssuedAlertsEnd({
  ...
  const content = isMultipleAreas
    ? `The ${formatAlertNameLower(issuedWarningOrAlert.headline, isMultipleAreas)} remain in place for ${formatAreasList(allAreas)}.`
    : `The ${formatAlertNameLower(issuedWarningOrAlert.headline, isMultipleAreas)} remains in place for ${formatAreasList(allAreas)} until ${`${DateTime.fromISO(issuedWarningOrAlert.expires).toFormat('HHmm')}hrs`} today.`;
