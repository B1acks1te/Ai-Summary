...
export class ScrapeService {
    ...
    try {
      ...
      const alerts = await Promise.all(
        feed.entry.map(async (entry) => {
          ...
          const { alert } = alertObj as { alert: Alert | undefined };
          if (!alert) {
            this.logger.error(
              `Malformed/empty alert response for ${entry.link.href}. Status: ${alertResponse.status}. Body snippet: ${alertData.slice(0, 300)}`,
            );
            return null;
          }
          if (alert.references) {
  ...
  private async fetchAlertById(id: string): Promise<Alert | undefined> {
    ...
    const { alert } = parser.parse(alertData) as { alert: Alert | undefined };

    if (!alert) {
      this.logger.error(
        `Malformed/empty alert response for id: ${id}. Status: ${alertResponse.status}. Body snippet: ${alertData.slice(0, 300)}`,
      );
    }
